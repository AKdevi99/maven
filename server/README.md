...
`SERVER_PORT` environment variable to the desired value:
   ```
   SERVER_PORT=8888 java -jar target/actifact-server.jar
   ``` 
...
1. Run the server application depending on the config profile. Below are the choices:
   * Default with no profile will use `config_file,instance_principals,resource_principal` authentication strategy
     ```bash
     java -jar target/actifact-server.jar
     ```
   * `test` profile will use user principal via oci config file authentication
     ```bash
     java -Dconfig.profile=test -jar target/actifact-server.jar
     ```
   * `prod` profile will use instance principal authentication
     ```bash
     java -Dconfig.profile=prod -jar target/actifact-server.jar
     ```
...
1. Build docker image
   ```
   docker build .. -t actifact-server -f Dockerfile
   ```
2. Run the application
   * Run from local machine using User Principal authentication. This will mount `~/.oci` as a volume in the running 
     docker container.
     ```
     docker run --user helidon --rm -p 8080:8080 -v ~/.oci:/home/helidon/.oci:ro actifact-server:latest
     ```
   * Run from an OCI instance using Instance Principal
     ```
     docker run --user helidon --rm -p 8080:8080  actifact-server:latest
     ```
...
[target/actifact-server-distribution.zip](target/actifact-server-distribution.zip)
...
```bash
$ scp target/actifact-server-distribution.zip user@DESTINATION_HOST:/home/user
```
3. Unzip (will extract to `actifact-server` directory) and run
```bash
$ unzip actifact-server-distribution.zip
$ java -jar actifact-server/actifact-server.jar
```